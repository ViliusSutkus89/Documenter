...
class DocumentListAdapter(
    private val filenameClickListener: (Document) -> Unit,
    ...
    class DocumentViewHolder(private val binding: ListItemDocumentBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(doc: Document, filenameClickListener: (Document) -> Unit, deleteClickListener: (Document) -> Unit) {
            binding.apply {
                document = doc
                filename.setOnClickListener {
                    filenameClickListener(doc)
                }
                deleteButton.setOnClickListener {
                    deleteClickListener(doc)
                }
    ...
    override fun onBindViewHolder(holder: DocumentViewHolder, position: Int) {
        holder.bind(getItem(position), filenameClickListener, deleteClickListener)
    }
}